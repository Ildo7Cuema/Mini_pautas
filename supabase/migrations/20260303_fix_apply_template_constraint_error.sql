-- ============================================
-- MIGRATION: Fix check_calculated_has_formula in apply_template
-- Purpose: Safely fallback is_calculated to false if formula is missing 
--          during template application to avoid constraint violation.
-- Date: 2026-03-03
-- ============================================
...
BEGIN
    ...
    LOOP
        -- Safely cast is_calculated based on formula presence
        IF v_componente.is_calculated = true AND (v_componente.formula_expression IS NULL OR trim(v_componente.formula_expression) = '') THEN
            v_safe_is_calculated := false;
        ELSE
            v_safe_is_calculated := v_componente.is_calculated;
        END IF;
...
COMMENT ON FUNCTION apply_template_to_turma IS 'Applies a discipline template to a turma, creating the discipline and all its components. Fixed check_calculated_has_formula constraint bypass.';
