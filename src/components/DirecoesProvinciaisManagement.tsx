/**
 * @component DirecoesProvinciaisManagement
 * @description Gestão de Direções Provinciais moderna mobile-first para SUPERADMIN
...
import {
    fetchAllDirecoesProvinciais,
    activateDirecaoProvincial,
    deactivateDirecaoProvincial,
    blockDirecaoProvincial,
    unblockDirecaoProvincial,
    type DirecaoProvincial
} from '../utils/superadmin'

interface DirecoesProvinciaisManagementProps {
    onNavigate?: (page: string) => void
    initialFilter?: 'all' | 'active' | 'inactive' | 'blocked' | 'attention'
}

export const DirecoesProvinciaisManagement: React.FC<DirecoesProvinciaisManagementProps> = ({ onNavigate, initialFilter }) => {
    const [direcoes, setDirecoes] = useState<DirecaoProvincial[]>([])
    ...
    const [selectedDirecao, setSelectedDirecao] = useState<DirecaoProvincial | null>(null)
    const [showBlockModal, setShowBlockModal] = useState(false)
    const [blockReason, setBlockReason] = useState('')
    const [actionLoading, setActionLoading] = useState<string | null>(null)
    const [showDetailsModal, setShowDetailsModal] = useState(false)
    const [detailsDirecao, setDetailsDirecao] = useState<DirecaoProvincial | null>(null)
    ...
    const loadDirecoes = async () => {
        try {
            ...
            const data = await fetchAllDirecoesProvinciais(filters)
            setDirecoes(data)
        } catch (err) {
            console.error('Error loading direções:', err)
            setError('Erro ao carregar direções provinciais')
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleActivate = async (id: string) => {
        setActionLoading(id)
        try {
            await activateDirecaoProvincial(id)
            await loadDirecoes()
        } catch (err) {
            showAlert('Erro', 'Erro ao activar direção provincial', 'error')
        } finally {
    ...
    const handleDeactivate = async (id: string) => {
        setActionLoading(id)
        try {
            await deactivateDirecaoProvincial(id)
            await loadDirecoes()
        } catch (err) {
            showAlert('Erro', 'Erro ao desactivar direção provincial', 'error')
        } finally {
            setActionLoading(null)
        }
    }

    const handleBlockClick = (direcao: DirecaoProvincial) => {
    ...
    const handleBlockSubmit = async () => {
        ...
        try {
            await blockDirecaoProvincial(selectedDirecao.id, blockReason)
            setShowBlockModal(false)
            setSelectedDirecao(null)
            setBlockReason('')
            await loadDirecoes()
        } catch (err) {
            showAlert('Erro', 'Erro ao bloquear direção provincial', 'error')
        } finally {
            setActionLoading(null)
        }
    }
    ...
    const handleUnblock = async (id: string) => {
        setActionLoading(id)
        try {
            await unblockDirecaoProvincial(id)
            await loadDirecoes()
        } catch (err) {
            showAlert('Erro', 'Erro ao desbloquear direção provincial', 'error')
        } finally {
            setActionLoading(null)
        }
    }

    const handleViewDetails = (direcao: DirecaoProvincial) => {
    ...
    return (
        <div className="min-h-screen bg-neutral-50 pb-24 md:pb-8">
            {/* Header with Gradient - Blue/Indigo for Provincial */}
            <div className="bg-gradient-to-r from-blue-700 via-blue-800 to-indigo-900 text-white px-4 py-6 md:px-8 relative overflow-hidden">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-blue-200 mt-1 text-sm md:text-base">
                                    Gerir todas as Direções Provinciais cadastradas
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            <div className="px-4 md:px-8 -mt-4 space-y-4">
                ...
                <div className="bg-white rounded-2xl p-3 shadow-sm border border-neutral-100 animate-slide-up">
                    <div className="relative">
                        ...
                        <input
                            type="text"
                            placeholder="Pesquisar por nome, província..."
                            value={searchQuery}
                            onChange={(e) => setSearchQuery(e.target.value)}
                            className="w-full pl-11 pr-4 py-3 border-0 bg-neutral-50 rounded-xl focus:ring-2 focus:ring-blue-500 transition-all text-sm"
                        />
                    </div>
                </div>
                        ...
                        <div className="hidden md:block bg-white rounded-2xl shadow-sm border border-neutral-100 overflow-hidden animate-slide-up">
                            <div className="overflow-x-auto">
                                <table className="table-excel w-full">
                                    <thead>
                                        <tr>
                                            <th className="text-left">Direção Provincial</th>
                                    ...
                                    <tbody>
                                        {filteredDirecoes.map((direcao) => (
                                            <tr key={direcao.id} className="hover:bg-neutral-50 transition-colors">
                                                ...
                                                <td className="text-left">
                                                    <div className="text-sm text-neutral-800">{direcao.provincia}</div>
                                                </td>
                            ...
                            {filteredDirecoes.length === 0 && (
                                <div className="text-center py-12">
                                    ...
                                    <p className="text-neutral-500">Nenhuma direção provincial encontrada</p>
                                </div>
                            )}
                        </div>
                ...
                <div className="fixed inset-0 bg-black/50 flex items-end md:items-center justify-center z-50 animate-fade-in">
                    <div className="bg-white w-full md:max-w-md md:rounded-2xl rounded-t-3xl p-6 md:mx-4 animate-slide-up max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <div>
                                <h2 className="text-xl font-bold text-neutral-800">Bloquear Direção Provincial</h2>
                        ...
                        <div className="bg-red-50 border border-red-200 rounded-xl p-3 mb-4">
                            <p className="text-sm text-red-800">
                                ⚠️ Esta acção irá impedir o acesso da Direção Provincial ao sistema.
                            </p>
                        </div>
                        ...
                        <div className="flex items-start justify-between mb-6">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-neutral-500 font-mono">{detailsDirecao.provincia}</p>
                                </div>
                            </div>
...
const StatusBadge: React.FC<{ direcao: DirecaoProvincial }> = ({ direcao }) => {
...
export default DirecoesProvinciaisManagement
